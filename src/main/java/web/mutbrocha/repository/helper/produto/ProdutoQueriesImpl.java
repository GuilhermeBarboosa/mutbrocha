...
//		p.fetch("pessoa", JoinType.INNER);
		...
		if (StringUtils.hasText(filtro.getSituacao())) {
			predicateList.add(builder.like(	builder.lower(p.<String>get("situacao")),
										"%" + filtro.getSituacao().toLowerCase() + "%"));
		}if (filtro.getCategoria() != null) {
			predicateList.add(builder.equal(p.<Categorias>get("categoria").<Long>get("id"),
					filtro.getCategoria()));
		}
