...
public class RelatorioService {
	...
	public byte[] gerarRelatorioSimplesTodasReservas() {
	...
	public byte[] gerarRelatorioComplexoTodasReservasPorUsuario() {

		try (Connection conexao = dataSource.getConnection()) {
				try {
					InputStream arquivoJasper = getClass().getResourceAsStream("/relatorios/RelatorioSQLDiretoComplexo.jasper");
					JasperPrint jasperPrint = JasperFillManager.fillReport(arquivoJasper, null, conexao);
					return JasperExportManager.exportReportToPdf(jasperPrint);
				} catch (JRException e) {
					logger.error("Problemas na geracao do PDF do relatório: " + e);
